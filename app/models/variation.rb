class Variation < ApplicationRecord
  ...
  scope :from_today, -> (date) { where("((? < variations.created_at + variations.spread * interval '1 day' AND ? > variations.created_at) OR variations.recurring = ?) AND variations.base = ?", date.end_of_day, date.end_of_day, true, false) }
